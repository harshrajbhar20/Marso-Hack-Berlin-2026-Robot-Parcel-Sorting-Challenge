...
Input: noisy_action (B, 16, 4)  +  global_cond (B, obs_horizon × 282)
  ...
  │       (128 = down_dims[0] for the final skip)
  │
...
|-----------|:----------:|
| ResNet18 Trunk (frozen at layer3) | ~11M |
| SpatialSoftmax + Linear | ~0.03M |
| Conditional U-Net 1D | ~12M |
| **Total (trainable)** | **~12M** |

Note: The ResNet18 trunk is fine-tuned during training (not frozen), so all ~11M ResNet parameters are trainable. The total trainable parameter count is approximately 23M.
...
We use 16 denoising steps at inference (same as training timesteps = 100 are subsampled to 16). More steps would produce slightly better actions but with diminishing returns and linearly increasing compute. 16 steps provides a good balance between action quality and inference speed.
